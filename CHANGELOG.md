...
- Portal do cliente (#263 / #300?#308): funcion�rios da rede fazem login em **/portal**, abrem e acompanham chamados da sua empresa, respondem no fio p�blico, anexam ficheiros, consultam a base de ajuda e recebem e-mail quando a equipe responde; o admin define a senha do portal no cadastro do funcion�rio
- Portal (#600, #601): cadastro de funcion�rio pelo modal da Rede permite definir senha do portal na cria��o; s�cio cadastra sem escolher empresas (escopo autom�tico em toda a rede)
- Portal (#604): RBAC por papel ? colaborador v� s� os pr�prios chamados; supervisor v� todos das empresas vinculadas; s�cio v� toda a rede
- Portal (#603): listagem e detalhe de atendimentos WhatsApp no `/portal` (somente leitura), com o mesmo escopo por papel e sem coment�rios internos da equipe
- Portal (#602): s�cio gere a equipa no `/portal` ? cadastro e edi��o de colaboradores e supervisores (empresas, senha do portal); outros s�cios listados com edi��o limitada; colaborador/supervisor recebem 403
- Portal (#605): branding white-label no `/portal` ? logo e cores da inst�ncia (mesmas configura��es de Configura��es ? Base de conhecimento); login e shell aplicam a identidade do contratante, n�o a marca DeskRudder
- Portal: shell alinhado ao painel DeskRudder (menu lateral com usu�rio/Sair, navbar com expandir/recolher); cor do menu lateral configur�vel (padr�o = navbar); chat ao vivo no `/portal` (mesmo canal da `/kb`, isolado por inst�ncia); login com logo em destaque, �cone de olho na senha e fundo minimalista; t�tulo do portal independente da central `/kb`
- WhatsApp (#593): ao **cadastrar** contacto no chat, o sistema sugere funcion�rios com nome semelhante para vincular (evita duplicados); o atendente pode ignorar e criar novo
- WhatsApp (#591): no detalhe da **Empresa**, nova aba **Chats** com atendimentos filtrados por aquela empresa (busca, pagina��o, abrir conversa / retomar)
- WhatsApp (#592): com funcion�rio em **mais de uma empresa**, o atendimento pode come�ar sem empresa; o atendente pergunta ao cliente e vincula (ou altera) a qualquer momento antes de encerrar ? 1 empresa continua autom�tica
- WhatsApp (#590): na listagem **Atendimentos**, cada card mostra a **empresa** do contacto (ou �Sem empresa� quando n�o houver v�nculo)
- Chat (#607): na aba **Atendendo**, sec��es **Comigo** e **Outros atendentes** quando o admin acompanha colegas; badge �Voc� e destaque visual nos seus; nome do respons�vel leg�vel nos de outros
- WhatsApp (#606): conversa mais utiliz�vel no **mobile** ? composer com campo largo e bot�o de manuais s� �cone; menu **?** com Transferir/Tickets; metadados em linha separada; painel de demandas colaps�vel; `safe-area` no composer

### Corre��es

- WhatsApp (#608): duas mensagens inbound seguidas do mesmo contacto deixam de abrir dois chats/protocolos ? lock por `wa_id` no webhook e uma �nica sequ�ncia de auto-mensagens por sess�o
- WhatsApp (#598): ap�s encerrar com avalia��o, janela de ~30 min (configur�vel) para a nota; se o cliente mandar outro texto, abre atendimento novo com essa mensagem; sem resposta, finaliza sozinho
