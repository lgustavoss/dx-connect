...
type Props = {
  ...
  /** Ícone estilo figurinha (dentro do campo) — #750. */
  varianteSticker?: boolean
}

export function RespostasProntasPicker({
  setorId,
  disabled,
  onInserir,
  modoComposer = false,
  varianteSticker = false,
}: Props) {
  ...
  const icone = varianteSticker ? (
    <svg className="size-5 shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.75" aria-hidden>
      ...
      <path d="M8 15c1.2 1.2 2.5 1.8 4 1.8s2.8-.6 4-1.8" strokeLinecap="round" />
    </svg>
  ) : (
