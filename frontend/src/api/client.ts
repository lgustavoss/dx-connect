...
export namespace PreTicketIa {
  export type Estado = 'rascunho' | 'analisado' | 'aprovado' | 'publicado' | 'descartado'
  export type Classificacao = 'bug' | 'melhoria' | 'spike' | 'infra' | 'documentacao' | 'duvida'
  export type Viabilidade = 'viavel' | 'nao_viavel' | 'precisa_contexto'

  export interface Analise {
    classificacao: Classificacao
    lacunas_perguntas: string[]
    riscos: string[]
  ...
  export interface Detalhe {
    ...
    rascunho_titulo?: string | null
    rascunho_corpo?: string | null
    rascunho_publicado_titulo?: string | null
    rascunho_publicado_corpo?: string | null
  ...
  export interface Metricas {
    ...
    uso: {
      ...
      sessoes_analisadas: number
      sessoes_aprovadas: number
    ...
    custo: {
      total_usd: number
      medio_por_analise_usd: number | null
      hoje_usd: number
    }
...
export const preTicketIa = {
  status: () =>
    api<{ ia_habilitada: boolean; github_habilitado: boolean }>('/pre-ticket/status'),
  listar: () => api<PreTicketIa.ListaItem[]>('/pre-ticket/sessoes'),
  get: (id: number) => api<PreTicketIa.Detalhe>(`/pre-ticket/sessoes/${id}`),
  historico: (id: number) => api<PreTicketIa.HistoricoItem[]>(`/pre-ticket/sessoes/${id}/historico`),
  criar: (data: PreTicketIa.Create) =>
    api<PreTicketIa.Detalhe>('/pre-ticket/sessoes', {
      method: 'POST',
      body: JSON.stringify(data),
    }),
  editarRascunho: (id: number, data: { rascunho_titulo?: string; rascunho_corpo?: string }) =>
    api<PreTicketIa.Detalhe>(`/pre-ticket/sessoes/${id}/rascunho`, {
      method: 'PATCH',
      body: JSON.stringify(data),
    }),
  analisar: (id: number) =>
    api<PreTicketIa.Detalhe>(`/pre-ticket/sessoes/${id}/analisar`, { method: 'POST' }),
  aprovar: (id: number) =>
    api<PreTicketIa.Detalhe>(`/pre-ticket/sessoes/${id}/aprovar`, { method: 'POST' }),
  publicarGithub: (id: number) =>
    api<PreTicketIa.Detalhe>(`/pre-ticket/sessoes/${id}/github`, { method: 'POST' }),
  descartar: (id: number) =>
    api<PreTicketIa.Detalhe>(`/pre-ticket/sessoes/${id}/descartar`, { method: 'POST' }),
  metricas: (params?: { desde?: string; ate?: string }) =>
    api<PreTicketIa.Metricas>(
      withParams('/pre-ticket/metricas', params as Record<string, string | undefined>),
    ),
};
