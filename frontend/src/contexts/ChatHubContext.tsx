...
export function ChatHubProvider({ children }: { children: ReactNode }) {
  ...
  const refreshContagens = useCallback(async () => {
    try {
      const [fila, meus] = await Promise.all([whatsappChats.fila(), whatsappChats.meus()])
      setFilaCount(fila.length)
      setAtendendoCount(meus.length)
    } catch {
  ...
  const value = useMemo(
    () => ({ busca, setBusca, filaCount, atendendoCount, refreshContagens }),
